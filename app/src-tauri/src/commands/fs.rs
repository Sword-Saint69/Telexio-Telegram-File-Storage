...
pub async fn cmd_upload_file(
    ...
    // Multi-part logic: 1.5GB chunks
    const CHUNK_SIZE: u64 = 1500 * 1024 * 1024;
    
    if size > CHUNK_SIZE {
        log::info!("File size {} > 1.5GB. Using Multi-Part Upload...", size);
        ...
        for part in 0..total_parts {
            ...
            let part_name = format!("{}.tdpart{}", std::path::Path::new(&path).file_name().unwrap().to_str().unwrap(), part + 1);
            let uploaded = client.upload_stream(&mut std::io::Cursor::new(chunk), n, part_name).await.map_err(map_error)?;
            let message = InputMessage::new().text(format!("[TD-PART:{}/{}]", part + 1, total_parts)).file(uploaded);
            client.send_message(&peer, message).await.map_err(map_error)?;
            
            if !tid.is_empty() {
                let percent = (((part + 1) as f64 / total_parts as f64) * 100.0) as u8;
...
pub async fn cmd_download_file(
    ...
    // Parallel Download logic (Max out bandwidth)
    let mut file = std::fs::File::create(&save_path).map_err(|e| e.to_string())?;
    let mut downloaded: u64 = 0;
    let mut last_percent: u8 = 0;

    // Grammers iter_download is serial by default. For true parallel, we need GetFile with offsets.
    // However, configuring grammers to use multiple connections internally is better.
    // For now, we optimize the loop and use a larger buffer.
