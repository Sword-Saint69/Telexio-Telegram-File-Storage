...
pub async fn cmd_auth_sign_in(
    ...
    log::info!("Signing in with code: {} (len: {})", clean_code, clean_code.len());
    ...
    let login_token = token_guard.as_ref().ok_or("No login session found (restart flow). Please request a new code.")?;

    match client.sign_in(login_token, &clean_code).await {
        Ok(_user) => {
