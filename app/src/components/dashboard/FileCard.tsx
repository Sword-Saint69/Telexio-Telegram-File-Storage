...
export function FileCard({ file, onDelete, onDownload, onPreview, isSelected, onClick, onContextMenu, onDrop, onDragStart, onDragEnd, activeFolderId, height }: FileCardProps) {
    ...
    // Lazy load thumbnail for image files using Intersection Observer
    useEffect(() => {
        if (isFolder || !isImageFile(file.name)) return;

        let cancelled = false;
        const observer = new IntersectionObserver((entries) => {
            if (entries[0].isIntersecting && !thumbnail && !thumbnailLoading) {
    ...
    }, [file.id, file.name, activeFolderId, isFolder, thumbnail, thumbnailLoading]);
    ...
    return (
        ...
        >
            <motion.div
                ...
                whileTap={{ scale: 0.98 }}
                className={`group cursor-pointer bg-telegram-surface rounded-xl overflow-hidden border transition-all relative
                ${isSelected ? 'border-telegram-primary bg-telegram-primary/5 shadow-lg ring-1 ring-telegram-primary' : 'border-telegram-border hover:border-telegram-primary/50 hover:shadow-md'}
                ${isDragOver ? 'ring-2 ring-telegram-primary bg-telegram-primary/20 scale-[1.02]' : ''}`}
            ...
            >
                ...
                {thumbnail ? (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center p-4">
                        {isFolder ? (
                            <Folder 
                                className={`w-12 h-12 ${isDragOver ? 'text-telegram-primary' : ''}`} 
                                style={file.color ? { color: file.color } : { color: 'var(--telegram-primary)' }}
                            />
                ...
                <div className={`absolute bottom-0 left-0 right-0 p-4 ${thumbnail ? 'text-white' : 'text-telegram-text'}`}>
                    <h3 className="text-sm font-semibold truncate w-full" title={file.name}>{file.name}</h3>
                    <p className={`text-xs font-medium mt-0.5 ${thumbnail ? 'text-white/70' : 'text-telegram-subtext'}`}>{file.sizeStr}</p>
                </div>
