...
import { ActivityCenter } from './dashboard/ActivityCenter';
...
export function Dashboard({ onLogout }: { onLogout: () => void }) {
    ...
    const { 
        activeFolderId, setActiveFolderId, folders, isSyncing, isConnected, store,
        handleLogout, handleSyncFolders, handleCreateFolder, handleFolderDelete, handleFolderPin, handleFolderColor
    ...
    useKeyboardShortcuts({
        ...
        onActivity: () => setIsActivityCenterOpen(prev => !prev),
        onToggleTheme: toggleTheme,
        onToggleCompact: () => setIsCompact(prev => !prev),
        onFontIncrease: () => setFontSize(prev => Math.min(prev + 1, 24)),
        onFontDecrease: () => setFontSize(prev => Math.max(prev - 1, 12)),
        onSearch: handleFocusSearch,
        onSelectAll: handleSelectAll,
        onDelete: handleKeyboardDelete,
        onEscape: handleEscape,
        onEnter: handleEnter,
        enabled: !previewFile && !playingFile && !pdfFile && !showMoveModal && !isCommandPaletteOpen
    });
    ...
    return (
        ...
        >
            ...
            <AnimatePresence>
                ...
                <CommandPalette
                    ...
                    onCommand={(cmd) => {
                        ...
                        if (cmd === 'search' || cmd === 'open') setIsCommandPaletteOpen(true);
                        if (cmd === 'new-folder') {
                            const name = prompt("Enter folder name:");
                            if (name) handleCreateFolder(name);
                        }
                        if (cmd === 'activity') setIsActivityCenterOpen(true);
                    }}
                />
                <ActivityCenter 
            ...
            <div 
                className={`flex flex-1 overflow-hidden ${isCompact ? 'compact-mode' : ''}`}
                style={{ '--font-size-base': `${fontSize}px` } as any}
                dir="auto"
            >
                <Sidebar
                    folders={folders}
