...
export function useTelegramConnection(onLogoutParent: () => void) {
    ...
    const handleFolderPin = async (folderId: number) => {
        if (!store) return;
        const updated = folders.map(f => f.id === folderId ? { ...f, is_pinned: !f.is_pinned } : f);
        setFolders(updated);
        await store.set('folders', updated);
        await store.save();
        ...
        if (folder?.is_pinned) {
            toast.success(`"${folder.name}" pinned to top.`);
        }
    };

    const handleFolderColor = async (folderId: number, color: string) => {
        if (!store) return;
        const updated = folders.map(f => f.id === folderId ? { ...f, color } : f);
        setFolders(updated);
        await store.set('folders', updated);
        await store.save();
    };
